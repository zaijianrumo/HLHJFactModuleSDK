//
//  HLHJCustomCameraViewController.m
//  HLHJFactModuleSDK
//
...
- (void)setupUI
{
    ...
    self.bottomView.backgroundColor = [[UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1] colorWithAlphaComponent:.9];
    ...
    self.dismissBtn.frame = CGRectMake(60, self.bounds.size.height/2-25/2, 25, 25);
    [self.dismissBtn setImage:[UIImage imageNamed:@"imgBundle.bundle/ic_bl_guanbi"] forState:UIControlStateNormal];
    [self.dismissBtn addTarget:self action:@selector(dismissVC) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.dismissBtn];
    
    self.cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.cancelBtn.backgroundColor = [[UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1] colorWithAlphaComponent:.9];
    [self.cancelBtn setImage:[UIImage imageNamed:@"imgBundle.bundle/retake"] forState:UIControlStateNormal];
    ...
    [self.doneBtn setImage:[UIImage imageNamed:@"imgBundle.bundle/takeok"] forState:UIControlStateNormal];
...
//--------------------------------------------------------//
//--------------------------------------------------------//
@interface HLHJCustomCameraViewController () <CameraToolViewDelegate, AVCaptureFileOutputRecordingDelegate>
{
    //拖拽手势开始的录制
    BOOL _dragStart;
    BOOL _layoutOK;
...
@implementation HLHJCustomCameraViewController
...
//        NSLog(@"allowTakePhoto与allowRecordVideo不能同时为NO");
    }
}
...
- (void)setupUI
{
    ...
    self.focusCursorImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"imgBundle.bundle/focus"]];
    ...
    [self.toggleCameraBtn setImage:[UIImage imageNamed:@"imgBundle.bundle/ic_bl_paizhao"] forState:UIControlStateNormal];
...
- (NSString *)transformSessionPreset
{
    switch (self.sessionPreset) {
        case HLHJCaptureSessionPreset325x288:
            return AVCaptureSessionPreset352x288;
            
        case HLHJCaptureSessionPreset640x480:
            return AVCaptureSessionPreset640x480;
            
        case HLHJCaptureSessionPreset1280x720:
            return AVCaptureSessionPreset1280x720;
            
        case HLHJCaptureSessionPreset1920x1080:
            return AVCaptureSessionPreset1920x1080;
            
        case HLHJCaptureSessionPreset3840x2160:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if (point.y > [UIScreen mainScreen].bounds.size.height-150-ZL_SafeAreaBottom) {//NOTE:!ZS_SafeAreaBottom
        return;
    }
...
- (NSString *)getVideoExportFilePath:(HLHJExportVideoType)type {
    
    NSString *format = (type == HLHJExportVideoTypeMov ? @"mov" : @"mp4");
...
- (void)captureOutput:(AVCaptureFileOutput *)output didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray<AVCaptureConnection *> *)connections error:(NSError *)error {
    if (CMTimeGetSeconds(output.recordedDuration) < 1) {
        if (self.allowTakePhoto) {
            ...
            [self.view makeToast:@"录制时间太短,请重新录制" duration:2.0 position:CSToastPositionCenter];
