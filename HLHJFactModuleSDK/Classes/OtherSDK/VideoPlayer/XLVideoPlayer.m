...
@property (nonatomic, strong) id timeObserver;//定时观察者
@end
...
- (UIView *)bottomBar {
    if (!_bottomBar) {
        ...
        [fullScreenBtn setImage:[UIImage imageNamed:@"imgBundle.bundle/btn_zoom_out"] forState:UIControlStateNormal];
        [fullScreenBtn setImage:[UIImage imageNamed:@"imgBundle.bundle/btn_zoom_in"] forState:UIControlStateSelected];
...
- (UIButton *)closeBtn{
    if (!_closeBtn) {
        ...
        [_closeBtn setImage:[UIImage imageNamed:@"ic_yuyue_gb"] forState:UIControlStateNormal];
//        [_closeBtn setEnlargeEdge:20];
        _closeBtn.layer.opacity = 0.0f;
...
- (UIButton *)playOrPauseBtn {
    if (!_playOrPauseBtn) {
        ...
        [_playOrPauseBtn setBackgroundImage:[UIImage imageNamed:@"imgBundle.bundle/ic_bl_bofang"] forState:UIControlStateNormal];
        [_playOrPauseBtn setBackgroundImage:[UIImage imageNamed:@"imgBundle.bundle/ic_bl_zanting"] forState:UIControlStateSelected];
...
//        debugLog(@"UIDeviceOrientationLandscapeLeft");
...
//    [self.playSuprView addSubview:self];
    ...
    if (self.bindTableView) {
        HLHJFactTableViewCell *cell = [self.bindTableView cellForRowAtIndexPath:self.currentIndexPath];
        [cell.picContainerView addSubview:self];
    }
...
- (void)show {
    [UIView animateWithDuration:kBarAnimateSpeed animations:^{
        ...
        self.closeBtn.layer.opacity = kOpacity;
...
- (void)hiden {
    ...
    [UIView animateWithDuration:kBarAnimateSpeed animations:^{
        ...
        self.closeBtn.layer.opacity = 0.0f;
    } completion:^(BOOL finished){
        if (finished) {
            self.barHiden = !self.barHiden;
        }
...
//        debugLog(@"totalBuffer：%.2f",totalBuffer);
